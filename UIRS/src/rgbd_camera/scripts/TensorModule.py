...
class Tensor():
    ...
    def __init__(self, labels, CAMERA_WIDTH = 1920, CAMERA_HEIGHT = 1080):
        ...
        path_to_detect = path = os.path.join(os.path.expanduser('~'), 'ROS_Works', 'UIRS','src', 'rgbd_camera','includes', 'detect.tflite')
    ...
    def get_dist_and_crop(self, res, frame, depth_frame):
        ...
        if res:
            ...
            if h>6 and w>6:
                ...
                if min_dist!=None:
                    dist = crop_depth[int(h/2), int(w/2)]
                    anglex = crop_angle_X[int(h/2)][int(w/2)]
                    anglez = crop_angle_Z[int(h/2)][int(w/2)]
                    self.distance_angle = [dist, anglex, anglez]
                    cv2.circle(crop_image, (int(w/2), int(h/2)), 5, (0, 0, 255), cv2.FILLED)      
                    #for i in range(np.size(crop_image, 0)):
                    #    for j in range(np.size(crop_image, 1)):
